#!/usr/bin/env python3
"""
Simplified data storage script
Handles both local archival and Kafka (if available)
"""
...
def archive_folder(folder_name, folder_path):
    ...
    try:
        ...
        logger.info(f"✓ Archived: {archive_name}.zip ({archive_size} bytes)")
...
def try_kafka_storage():
    ...
    try:
        ...
        logger.info("\n" + "="*70)
        logger.info("ATTEMPTING KAFKA STORAGE")
        logger.info("="*70 + "\n")
        
        KAFKA_BROKER = 'localhost:9092'
        KAFKA_TOPICS = {
            'optimize_delivery': 'optimize-delivery-results',
            'inventory_management': 'inventory-management-results'
        }
                    ...
                    for file_name in os.listdir(folder_path):
                        ...
                        if os.path.isfile(file_path):
                            try:
                                ...
                                producer.send(KAFKA_TOPICS[folder_key], value=message)
                                total_sent += 1
                                logger.info(f"  ✓ Sent {file_name} to Kafka")
                            except Exception as e:
                                logger.warning(f"  ⚠ Skipped {file_name}: {e}")
            
            producer.flush()
            producer.close()
            logger.info(f"\n✓ Sent {total_sent} files to Kafka")
            return True
            
        except Exception as e:
            logger.warning(f"⚠ Kafka unavailable: {e}")
            logger.info("  (This is optional - archival completed successfully)")
            return False
            
    except ImportError:
        logger.info("ℹ Kafka module not installed (optional)")
        logger.info("  Install with: pip3 install kafka-python --break-system-packages")
        return False
